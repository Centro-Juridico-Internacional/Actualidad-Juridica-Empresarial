...
export function getNews({ categoryId }: { categoryId: string }) {
	return query(
		`news?filters[categorias][slug][$contains]=${categoryId}&populate[imagenes][fields][0]=url&populate[autor][populate][avatar][fields][0]=url&populate[autor][fields][1]=name&populate[categorias][fields][2]=name`
	).then((res) => {
		const { data, meta } = res;
